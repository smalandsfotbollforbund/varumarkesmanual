---
description: >-
  Vad vi kallar oss själva och andra. Bindande för att skapa tydlighet för
  mottagaren och konsekvent bevara de olika varumärkena.
---

# Namnbruk och samprofilering

<details open>

<summary><strong>Smålands Fotbollförbund</strong></summary>

...är alltid namnet, och skrivs alltid så vid första omnämnandet.

Förkortningar (SmFF, Smålands FF, SDF osv) undviks och används aldrig officiellt. I löpande text efter definitionen (Smålands Fotbollförbund) används därefter: **distriktet**, **Småland** (enbart i fotbollssammanhang) eller om lämpligt **vi**, **för oss** eller liknande benämningar.

</details>

<details open>

<summary><strong>Smålandsfotbollen</strong></summary>

...benämner allt inom småländsk fotboll: föreningarna, spelarna, ledarna, domarna och distriktet tillsammans. Inte enbart organisationen.

</details>

<details open>

<summary><strong>Förbundet</strong></summary>

...betyder alltid Svenska Fotbollförbundet, som ibland också benämns **SvFF**. Smålands Fotbollförbund kallas aldrig "förbundet", utan refereras snarare till som **distriktet**.

</details>

<details open>

<summary><strong>RF-SISU Småland</strong></summary>

...skrivs alltid i exakt den formen, med bindestreck och Småland. Övriga förkortningar som DF används aldrig.

**Riksidrottsförbundet** benämns också som **RF**, men avser riksorganisationen.

</details>

{% hint style="info" icon="circle-info" %}
Som strikt undantag är det, av varumärkesägaren, godkänt att använda smalandsff som handler för sociala medier med hänvisning till om smalandsfotbollforbund är för långt för plattformen.
{% endhint %}

{% hint style="info" icon="circle-info" %}
Som strikt undantag är det, av varumärkesägaren, godkänt att använda smalandsfotbollen som domän med hänvisning till historia.
{% endhint %}

### Två lägen styr all kommunikation

Varumärket är strikt distriktets eget. Kopplingen till Svenska Fotbollförbundet och RF-SISU Småland är infrastruktur, aldrig identitet. Allt som hämtas från förbundet behåller sitt namn eller ges ett namn med tydlig nationell koppling, så att ursprunget alltid syns.

{% columns %}
{% column %}
#### Distriktsläget

[Smålandsröd](../visuell-identitet/farg.md#primarpaletten) primär, egen [röst](../rost/), egna [element](../visuell-identitet/). Detta används i de format som är möjliga att anpassa för att tydliggöra avsändaren.
{% endcolumn %}

{% column %}
#### Förbundsläget

[Förbundets](namnbruk-och-samprofilering.md#forbundet) plattformar, till exempel hemsidan, där förbundets riktlinjer styr och distriktets färgpalett inte kan användas på plattformsstyrda ytor. [Sverigeblå](../visuell-identitet/farg.md#kopplingsfarg-anvands-aldrig-i-distriktslaget) reserveras för förbundskopplat material.
{% endcolumn %}
{% endcolumns %}

#### Delad infrastruktur mellan lägena

Det typografiska systemet som innehåller teckensnittet SvFF Trim och Stag Sans med Calibri som fallbacklösning i Microsoft Office-miljö delar distriktsläget med förbundsläget. Detta styrs av förbundets typografiska riktlinjer.

Anledningen härleds dels till att visa på det gemensamma, samhörigheten och kopplingen mellan de två lägena, samt av tekniska anledningar för t.ex. den digitala infrastrukturen som hemsidan.
